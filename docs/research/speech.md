...
Follow-up: [speech onset latency investigation and implementation, 2026-09-08](speech-latency.md) measures the worker on the current M1 Max, separates cold initialization from playback startup, and records background preparation, a persistent output stream, conservative silence trimming, and measured first-buffer latency.
...
- A persistent Python subprocess performs synthesis. A **separate persistent sounddevice/PortAudio subprocess** keeps one output stream open across chunks. Pause keeps the PCM position and sends silence without advancing it; resume continues there. Stop clears the current buffer immediately, subject to the small device buffer, without closing the stream. Off/shutdown close owned processes. No `SIGSTOP` approximation is used for audio pause.
- Automatic runs warm synthesis and the output device without blocking the agent. One first chunk is speculatively generated while text streams, with at most one speculative request in flight. Final rendering and settings must match before reuse; narration still waits for `agent_settled`. Stop suppresses the current run's automatic narration. Exact initial zero samples are trimmed with 20 ms retained before the first nonzero sample. The playing state waits for the first-buffer acknowledgement.
