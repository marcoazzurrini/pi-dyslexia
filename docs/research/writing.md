...
**Status: research note with an approved trial implementation in version 0.2.0.** The active policy is the adapted [`extensions/SKILL.md`](../../extensions/SKILL.md), not the initial replacement proposal below. The owner chose to retain Caveman's core Rules and Auto-Clarity sections, apply targeted fixes, and remove intensity levels. Benefits remain untested.
...
This historical candidate records the first synthesis. It is not injected at runtime; [`extensions/SKILL.md`](../../extensions/SKILL.md) is the single active writing policy:
...
- **Control cleanup, version 0.3.0:** use `/dyslexia on|off|status` and the `dyslexia: on/off` footer. Remove Caveman-specific activation text from `index.ts`; keep the writing policy and its attribution unchanged. The old `/dyslexia caveman ...` syntax reports the new usage without changing state.
- **Response-length trial, issue #1:** the owner reports that soft instructions still produce overly long answers, especially for research. Put total response length before stylistic rules and trial a target of under 120 words of chat prose per turn. Research defaults to a synthesis retaining main conclusions, decision-changing evidence and citations, conditions, uncertainty, and warnings. Explicitly requested detail and essential completeness, clarity, or safety can exceed the target; code and requested documents remain intact. This number is a personal product preference, not an evidence-based threshold. Hard enforcement and rewriting hooks remain deferred. Prompt-injection checks do not establish model adherence; evaluate actual response length and omissions during use.
- Regression checks cover extension-policy loading, preserved safeguards, on/off behavior, session resets, invalid files, and package contents. A loader smoke check verifies integration; the owner's fresh-session trial is still required to evaluate actual answers.
